...
import kotlinx.android.synthetic.main.climb_fifteen_screen.*

class ClimbFifteenScreen: AppCompatActivity(){

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.climb_fifteen_screen)

        backFifteen.setOnClickListener {
            startActivity(Intent(this, ClimbList::class.java))
        }

        val chk1 = findViewById<CheckBox>(R.id.checkBoxFifteen)
        val database = getSharedPreferences("database", Context.MODE_PRIVATE)
        val checkMark = getSharedPreferences("checking", Context.MODE_PRIVATE)
        val isChecked = checkMark.getBoolean("climbCheckFifteen", false)
        ...
        chk1.setOnClickListener { v ->
            ...
            if (box1) {
                ...
                checkMark.edit().apply {
                    putBoolean("climbCheckFifteen", true)
                }.apply()
            }
            else {
                savedScore -= 1000
                ...
                checkMark.edit().apply {
                    putBoolean("climbCheckFifteen", false)
                }.apply()
            }
        }
