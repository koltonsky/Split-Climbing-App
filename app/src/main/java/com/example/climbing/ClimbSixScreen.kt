...
import kotlinx.android.synthetic.main.climb_six_screen.*

class ClimbSixScreen: AppCompatActivity(){

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.climb_six_screen)

        backSix.setOnClickListener {
            startActivity(Intent(this, ClimbList::class.java))
        }

        val chk1 = findViewById<CheckBox>(R.id.checkBoxSix)
        val database = getSharedPreferences("database", Context.MODE_PRIVATE)
        val checkMark = getSharedPreferences("checking", Context.MODE_PRIVATE)
        val isChecked = checkMark.getBoolean("climbCheckSix", false)
        ...
        chk1.setOnClickListener { v ->
            ...
            if (box1) {
                ...
                checkMark.edit().apply {
                    putBoolean("climbCheckSix", false)
                }.apply()
            }
        }
