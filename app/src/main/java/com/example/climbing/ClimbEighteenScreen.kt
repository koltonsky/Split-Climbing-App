...
import kotlinx.android.synthetic.main.climb_eighteen_screen.*

class ClimbEighteenScreen: AppCompatActivity(){

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.climb_eighteen_screen)

        backEighteen.setOnClickListener {
            startActivity(Intent(this, ClimbList::class.java))
        }

        val chk1 = findViewById<CheckBox>(R.id.checkBoxEighteen)
        val database = getSharedPreferences("database", Context.MODE_PRIVATE)
        val checkMark = getSharedPreferences("checking", Context.MODE_PRIVATE)
        val isChecked = checkMark.getBoolean("climbCheckEighteen", false)
        ...
        chk1.setOnClickListener { v ->
            ...
            if (box1) {
                ...
                checkMark.edit().apply {
                    putBoolean("climbCheckEighteen", true)
                }.apply()
            }
                ...
                checkMark.edit().apply {
                    putBoolean("climbCheckEighteen", false)
                }.apply()
            }
        }
