...
class ClimbOneScreen: AppCompatActivity(){

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.climb_one_screen)

        backOne.setOnClickListener {
            startActivity(Intent(this, ClimbList::class.java))
        }

        val chk1 = findViewById<CheckBox>(R.id.checkBoxOne)
        val database = getSharedPreferences("database", Context.MODE_PRIVATE)
        val checkMark = getSharedPreferences("checking", Context.MODE_PRIVATE)
        val isChecked = checkMark.getBoolean("climbCheckOne", false)
        ...
        chk1.setOnClickListener { v ->
            ...
            if (box1) {
                ...
                checkMark.edit().apply {
                    putBoolean("climbCheckOne", false)
                }.apply()
            }
        }
